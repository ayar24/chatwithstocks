	...
	if ($sac_no_js && $sac_verify) {
		...
		if (!empty($sac_user_name) && !empty($sac_user_text) && $sacSendChat === 'yes') {
			...
			$sac_user_url  = apply_filters('sac_process_chat_url',  $sac_user_url);
 			...
 			$current_credits = get_user_meta( get_current_user_id(), 'wps_wallet', true ); 
			
			if (intval($current_credits) > 0)
	 			update_user_meta( get_current_user_id(), 'wps_wallet', intval($current_credits) - 1 ); 
			else 
				wp_die("Not enought credits, please load more credit", 200);
			
			do_action('sac_process_chat', $sac_user_name, $sac_user_text, $sac_user_url);
			sac_addData($sac_user_name, $sac_user_text, $sac_user_url);
			//sac_deleteOld();
			$url = 'https://chatwithstocks.azurewebsites.net/hello';
			$input_message = $sac_user_text;
			$query_string = http_build_query(['message' => $input_message]);
